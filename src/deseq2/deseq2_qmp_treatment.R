...
# run DESeq
design(dds_res) <- ~ batch + timepoint + treatment
dds_res <- DESeq(dds_res, test="Wald")
saveRDS(dds_res, snakemake@output[["dds"]])

resultsNames(dds_res)

paste("qmp wald with padj<0.1")
res_group_0.1 <- results(dds_res, lfcThreshold = 1, alpha = 0.1)
summary(res_group_0.1)

paste("qmp wald with padj<0.05")
